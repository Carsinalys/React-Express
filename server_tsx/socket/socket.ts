import { newMessage, Message, Room } from '../interfaces/interfaces';
...
const socket = server => {
  ...
  io.on("connection", async function(socket) {
    ...
    socket.on("messageFromReact", async (data: newMessage) => {
      const newMessage: Message = await Message.create(data);
      io.emit("messageToState", newMessage);
      io.emit("messageToNav", newMessage);
    });
    socket.on("disconnect", async () => {
      const curRooms: Room[] = await Rooms.find();
    ...
    curRooms.forEach(item => {
      socket.on(item.name, (id: string, msg: any) => {
